...
SELECT count(pedido_id), status FROM pedidos
WHERE NOT status = 'cancelado'
GROUP BY statu
```
🔹 Desafio 2: Mostrar todos os pagamentos que NÃO foram feitos via 'boleto'
``` sql
SELECT * FROM pagamentos LIMIT 3

SELECT pagamento_id, metodo AS "Forma de Pagamento"
FROM pagamentos
...
SELECT pedido_id, status AS "Status do Pedido"
FROM pedidos
WHERE status IN ('entregue', 'enviado')
```
🔹 Desafio 2: Mostrar pagamentos feitos com 'pix' ou 'cartao_credito'
``` sql
SELECT pagamento_id, metodo AS "Formas de Pagamentos" FROM pagamentos
...
SELECT pagamento_id, metodo AS "Formas de Pagamentos"
FROM pagamentos
WHERE metodo IN ('pix', 'cartao_credito')
```
🔹 Desafio 2: Mostrar pedidos com status que NÃO sejam "pendente" ou "confirmado"
``` sql
SELECT produto_id, nota AS "Avaliação do Produto"
FROM avaliacoes
...
```
🔹 Desafio 2: Mostrar pagamentos realizados no ano de 2025
``` sql
SELECT * FROM pagamentos LIMIT 5

SELECT pagamento_id, valor, data_pagamento FROM pagamentos 
...
WHERE descricao LIKE '%casa%'
```
🔹 Desafio 2: Mostrar avaliações cujo comentário contenha a palavra "qualidade"
``` sql
SELECT * FROM avaliacoes limit 3

SELECT produto_id, comentario FROM avaliacoes 
...
WHERE estado LIKE 'S%'
```
🔹 Desafio 2: Mostrar clientes com email de dominio 'example' ou 'exemple' com o final .net.
``` sql
...
SELECT pedido_id, status AS "Status do Pedido", data_pedido AS "Data do Pedido"
FROM pedidos
...
SELECT pedido_id AS "Id do Pedido", metodo AS "Método de Pagamento", valor AS "Valor (R$)", status AS "Status do Pagamento"
FROM pagamentos
WHERE metodo LIKE 'cartao%'
AND valor BETWEEN 100 AND 1000
AND status NOT IN ('recusado')
...
SELECT nome AS "Nome do Produto", marca AS "Fabricante", preco AS "Valor (R$)", estoque AS "Estoque do Produto"
FROM produtos
